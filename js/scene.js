...
export class Scene {
    ...
    update() {
        ...
        if (this.exploding) {
            if ((this.transition += Scene.TRANSITION_SPEED_EXPLOSION) > 1)
                this.transition = 1;
        }
    ...
    render(time) {
        ...
        UniformBlocks.GLOBALS.setTransition(this.exploding ? Math.sin(transition * Math.PI * .5) : transition);
